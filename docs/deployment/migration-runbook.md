...
| _pending_  | `0009_campaign_pipeline` (adds `campaign_requests`, `campaign_assets`, `campaign_review_events`, `publish_jobs` — 4 new tables, expect 30) — no live read path reads these until Phase 3 PR2, so the apply can land after PR #16 merges but **must** precede PR2 | production shared Neon Postgres | —                                                                                         | founder |
