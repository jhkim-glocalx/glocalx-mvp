...
import { AppStage } from "@/app/_components/app-stage"
...
import { StepNavigation, type StepItem } from "@/app/_components/step-navigation"

const appSteps = [
  { eyebrow: "가게정보 · 연동", id: "onboarding", label: "온보딩" },
  { eyebrow: "GBP 초안 · 발행", id: "post", label: "포스팅" },
...
export function AppWorkspace() {
  const [activeStepId, setActiveStepId] = useState("post")
  ...
  async function handlePublish() {
    ...
    try {
      ...
      setPublish({
        kind: "blocked",
        message:
          error instanceof Error ? error.message : "게시 상태를 확인하지 못했습니다.",
      })
    }
  }
      ...
      <PhoneFrame>
        {activeStepId === "onboarding" ? (
          <div className="grid gap-3">
            ...
            <StatusCard label="매장" status="success" value="브런치모먼트 홍대점" />
          </div>
        ) : (
          <div className="grid gap-4">
            ...
            <form className="grid gap-3" onSubmit={handleDraft}>
              ...
              <ActionChip
                ...
                label={draft.kind === "loading" ? "초안 생성 중" : "GBP 초안 만들기"}
              />
            </form>
            {draft.kind === "ready" ? (
              <div className="grid gap-3">
                <StatusCard label="초안 준비" status="success" value="DRAFT_READY" />
                <p className="rounded-2xl bg-white p-4 text-sm font-bold leading-6 text-[var(--ink)]">
                  {draft.koreanCopy}
                </p>
                <ActionChip
                  disabled={publish.kind === "loading"}
                  label={publish.kind === "loading" ? "게시 확인 중" : "GBP 게시하기"}
                  onClick={handlePublish}
                />
              </div>
            ) : null}
            {draft.kind === "error" ? (
              <p className="gx-inline-feedback text-sm font-bold">{draft.message}</p>
            ) : null}
            {publish.kind === "published" ? (
              <StatusCard label="게시 상태" status="success" value={publish.message} />
            ) : null}
